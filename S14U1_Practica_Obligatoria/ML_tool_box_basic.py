...
def describe_features(df:pd.DataFrame,umbral_categoria=10, umbral_continua=30.0) -> pd.DataFrame:
...
def lista_features(df, umbral_categoria=10, umbral_continua=30):
...
def get_features_num_regression(df:pd.DataFrame, target_col:str, umbral_corr:float, pvalue:float=None, umbral_card_target=10.0, umbral_categorias_feature=10, pearson_results=False) -> list:
    """
...
def bivariante_hist_scatter_combined_with_regression(df, target_col, umbral_corr=0.0, pvalue= None, umbral_card_target=10.0, umbral_categorias_feature=10, pearson_results=True, f_regression=True):
...
def f_regression_analysis(df, target_col, umbral_categorias_feature=10, umbral_card_target=10.0):
    
####### FALTA PONER EL CONTROL DE ERROR DE LA TARGET, ENTRE OTROS #########
...
def get_features_cat_regression(df:pd.DataFrame, target_col:str, pvalue=0.05, umbral_categoria=10, umbral_card=10.0, mann_results=False, anova_results=False) -> list:
    """
...
def categoricas_hue(df, target_col, columnas_categoricas):
