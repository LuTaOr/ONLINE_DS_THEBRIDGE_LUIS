...
def corr_features_num(df, columnas_num, umbral, figsize=(12,6)):
    # Calcular la matriz de correlación
    correlacion_features_num = df[columnas_num].corr()

    # Filtrar la matriz de correlación según el umbral proporcionado
    filtrado = correlacion_features_num[(correlacion_features_num > umbral) | (correlacion_features_num < -umbral)]

    # Reemplazar NaN con un guion '-' en el DataFrame filtrado
    filtrado_str = filtrado.fillna('-')

    # Formatear los valores numéricos a dos decimales y reemplazar NaN por '-'
    annot_data = filtrado.stack().map(lambda x: f"{x:.2f}" if isinstance(x, float) else '-').unstack()

    # Aumentar el tamaño de la figura
    plt.figure(figsize=figsize)  # Ajusta el tamaño según tus necesidades

    # Crear un DataFrame para el heatmap con NaN donde están los guiones
    heatmap_data = filtrado.copy()  # Copia del DataFrame original para el heatmap
    heatmap_data[~((heatmap_data > umbral) | (heatmap_data < -umbral))] = np.nan  # Mantener solo los valores relevantes

    # Crear el heatmap solo con los valores filtrados
    sns.heatmap(heatmap_data,
                 annot=annot_data,  # Mostrar los valores de correlación o guiones formateados
                 cmap="coolwarm",      # Paleta de colores
                 fmt="",               # Formato vacío ya que se usan strings en annot
                 square=True,         # Hacer las celdas cuadradas
                 linewidths=0.5)      # Añadir líneas entre las celdas

    plt.title('Matriz de Correlación', fontsize=20)  # Título del gráfico
...
def REGRE_FN_plot_features_num_regression(df:pd.DataFrame, target_col='', columns=[], umbral_corr=0.0, pvalue=None, umbral_card=10.0) -> list:
    ...
    lista = REGRE_FN_get_features_num_regression(df, target_col, umbral_corr, pvalue, umbral_card)
...
def REGRE_FC_plot_features_cat_regression(df:pd.DataFrame, target_col= "", columns=[], pvalue=0.05, with_individual_plot=False, umbral_categoria=10, umbral_card=10.0) -> list:
    ...
    lista = REGRE_FC_get_features_cat_regression(df, target_col, pvalue, umbral_categoria, umbral_card)
...
def Uni_Num_hist_box_plot(df, columns, whisker_width=1.5, bins = None):
...
def CLASI_BI_FeaNum_hist_kde_plot(df, features_num_iniciales, target, num_graf_per_line=2, show_kde_only=False):
    ...
    # Definir una paleta de colores más fuertes y diferenciados
    palette = sns.color_palette("Set2", n_colors=len(df[target].unique()))  # Paleta Set2 es más vibrante

    for i, col in enumerate(features_num_iniciales):
        if show_kde_only:
            # Solo mostrar KDE
            sns.kdeplot(data=df, x=col, hue=target, ax=axes[i], palette=palette)
        else:
            # Mostrar histograma y KDE
            sns.histplot(data=df, x=col, hue=target, ax=axes[i], kde=True, palette=palette)

        axes[i].set_title(f'{"Curva KDE de" if show_kde_only else "Histograma Bivariante de"} {col}')
...
def Uni_Cat_bar_plot(df, columnas_categoricas, relativa=False, mostrar_valores=False):
    num_columnas = len(columnas_categoricas)
    num_filas = (num_columnas // 2) + (num_columnas % 2)
    fig, axes = plt.subplots(num_filas, 2, figsize=(15, 5 * num_filas))
    axes = axes.flatten()
